...
{
  private readonly identities: D1IdentityTransitions;

  constructor(private readonly database: D1Database) {
    this.identities = new D1IdentityTransitions(database);
  }
  ...
  async createRequest(input: CreateHelpRequest): Promise<CreateHelpRequestOutcome> {
    ...
    const { twitchLogin, statements } = await this.identities.prepareRequestMapping({
  ...
  async upsertUserMapping(input: {
    ...
    if (input.twitchUserId !== undefined) {
      await this.identities.assertNoStableIdentityCollision(twitchLogin, input.twitchUserId);
    }
    const statements = this.identities.mappingStatements({
      twitchLogin,
      ...(input.twitchUserId === undefined ? {} : { twitchUserId: input.twitchUserId }),
      ...(input.discordUserId === undefined ? {} : { discordUserId: input.discordUserId }),
      ...(input.discordDisplayName === undefined
  ...
  observeTwitchIdentity(input: {
    ...
    return this.identities.observe(input);
  }
