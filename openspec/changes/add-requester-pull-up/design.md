...
Active raids do not show `Cancel review`. They retain their result controls and missing-message recovery behavior.
...
- [A stale Cancel review control closes a newer or active message] → Include the source message ID in the interaction and clear only the matching canonical planned-review link.
...
3. Refresh the canonical board and detail messages to expose the direct selector and `Cancel review` control, dismiss missing planned reviews, and recover missing active details.
