...
- Add a Discord-only `/stats` command for the configured streamer and volunteer-sherpa role.
...
- Add a Discord-only staff `/users` command that shows Twitch, Discord, and Escape from Tarkov identity state in an ephemeral ten-user page with stateless Previous and Next controls.
...
- Query authoritative request, membership, and raid history directly unless local D1 benchmark evidence through 100,000 requests shows that aggregate storage is necessary.
...
- `mvp-deployment`: Register and validate `/stats` and `/users` and require local D1 performance evidence for both read paths.
...
- D1 repository aggregate queries over help requests, completed memberships, and successful raid groups.
