...
The system SHALL verify platform signatures and a ten-minute timestamp window before parsing private data. It SHALL validate complete Twitch request arguments in memory before any D1 operation. Invalid Twitch request guidance SHALL be best-effort and SHALL NOT claim a delivery receipt, observe identity, run receipt maintenance, or otherwise access D1. The system SHALL retain completed delivery protection for 24 hours and purge expired receipts only through bounded leased background maintenance after valid persisted commands. Discord request creation SHALL use its source-delivery uniqueness instead of storing a redundant mutation receipt. Every other Discord mutation SHALL either commit its completion receipt atomically with the guarded D1 mutation or retain an incomplete retryable receipt until the durable mutation succeeds. For valid commands, Twitch SHALL check for an exact stored delivery before it performs command-side D1 work, SHALL store a new reply with the command result atomically, and SHALL permit only the stored winner to send a reply or synchronize the board. A failed persisted Twitch reply SHALL remain eligible for one atomically claimed retry.

#### Scenario: Twitch request arguments are invalid
- **WHEN** a signed Twitch request command is missing a mode, map, or valid goal
- **THEN** the bot sends best-effort guidance without reading or writing D1, even when receipt cleanup is due
